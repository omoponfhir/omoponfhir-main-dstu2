...
pipeline{
    ...
    stages{
            ...
            steps{
                ...
                script{
                    docker.withRegistry('https://gt-build.hdap.gatech.edu'){
                        ...
                        def omoponfhirImage = docker.build("omoponfhir2:${env.BUILD_NUMBER}", "-f Dockerfile .")
        ...
        stage('Notify'){
            steps{
                script{
                    rancher confirm: true, credentialId: 'gt-rancher-server', endpoint: 'https://gt-rancher.hdap.gatech.edu/v2-beta', environmentId: '1a7', environments: '', image: "gt-build.hdap.gatech.edu/omoponfhir2:${env.BUILD_NUMBER}", ports: '', service: 'OMOPonFHIR/omoponfhir2', timeout: 50
                }
            }
        }
    }
